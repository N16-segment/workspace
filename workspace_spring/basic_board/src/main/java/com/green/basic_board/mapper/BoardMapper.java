...
//게시글 목록 조회 '쿼리'를 실행시킬 메서드
public List<BoardDTO> selectBoardList();

//하나의 게시글 정보를 조회하는 쿼리를 실행시킬 메서드
// ->즉, mapper 인터페이스에서는 쿼리를 실행시킬 추상 메서드를 만든다.
public BoardDTO getBoard(int boardNum);

//게시글 등록 쿼리 실행 메서드 (빈값을 채워주는 매개변수);
//select를 제외한
 //리턴타입이 등록이 되면서 변경되는 행의 개수이므로 숫자 -> int 로 리턴타입을 준다.
